...
//
////    /**
////     * 优化图结构，将连续的单链路径（串行路径）合并成一个节点集合
////     * @return 优化后的新图
////     */
////    public Graph<Node<T>> optimizeGraph() {
////        Graph<Node<T>> optimizedGraph = new Graph<>();
////        Map<T, Node<T>> nodeToGroup = new HashMap<>();
...
////
////            // 找到可以合并的路径起点
////            // 起点的条件：入度为0，或者入度不为1，或者出度不为1
////            if (getInDegree(node) != 1 || getOutDegree(node) != 1) {
////                List<T> path = new ArrayList<>();
////                collectPath(node, path, visited, nodeToGroup);
////            }
////        }
////
////        // 构建优化后的图
////        for (T node : getAllNodes()) {
////            Node<T> group = nodeToGroup.get(node);
////
////            // 添加原节点所在组到新图
////            optimizedGraph.addNode(group);
////
////            // 添加边：从当前组到其邻居组
////            for (Edge<T> edge : getNeighbors(node)) {
////                T target = edge.getTarget();
////                Node<T> targetGroup = nodeToGroup.get(target);
////
////                // 避免在同一组内添加边
////                if (!group.equals(targetGroup)) {
////                    optimizedGraph.addEdge(group, targetGroup, edge.getData());
////                }
////            }
////        }
////
...
////     * @param nodeToGroup 节点到组的映射
////     */
////    private void collectPath(T currentNode, List<T> path, Set<T> visited, Map<T, Node<T>> nodeToGroup) {
////        // 将当前节点加入路径
////        path.add(currentNode);
////        visited.add(currentNode);
////
////        // 检查是否可以继续延伸路径
////        if (getOutDegree(currentNode) == 1 && getInDegree(getNeighbors(currentNode).iterator().next().getTarget()) == 1) {
////            // 获取唯一的下一个节点
////            T nextNode = getNeighbors(currentNode).iterator().next().getTarget();
////
////            // 如果下一个节点的出度也是1，则继续延伸路径
////            if (getOutDegree(nextNode) == 1) {
////                collectPath(nextNode, path, visited, nodeToGroup);
////            } else {
////                // 否则终止路径，将下一个节点加入路径
////                path.add(nextNode);
...
////
////        // 将路径中的所有节点映射到同一个组
////        List<T> group = new ArrayList<>(path);
////        SuperNode<T> groupNode = new SuperNode<>();
////        for (T node : group) {
////            groupNode.addNode(node);
////        }
////
////        for (T node : path) {
////            nodeToGroup.put(node, (Node<T>) groupNode);
////        }
////
////        // 清空路径，为下一次收集做准备
////        path.clear();
...
//         * 执行图优化
//         */
//        public Graph_bak<T> optimize() {
//            // 创建新图
//            Graph_bak<T> optimizedGraph = new Graph_bak<>();
//
//            // 找出所有串行路径
...
//         * 构建优化后的图
//         */
//        private void buildOptimizedGraph(Graph_bak<T> optimizedGraph, List<List<T>> serialPaths) {
//            // 为每个串行路径创建合并节点
...
//         */
//        private Object createMergedNodeRepresentation(List<T> path) {
//            // 这里只是一个示例，实际需要根据T的具体类型来实现
//            StringBuilder sb = new StringBuilder("Merged:");
//            for (T node : path) {
//                sb.append(node).append("-");
//            }
//            return sb.substring(0, sb.length() - 1); // 移除最后的连字符
//        }
//
//        /**
//         * 判断节点是否是任何串行路径的一部分
...
//         * 重新连接边
//         */
//        private void reconnectEdges(Graph_bak<T> optimizedGraph, List<List<T>> serialPaths) {
//            // 处理非串行路径节点之间的边
...
//
//    public static void main(String[] args) {
//        Graph_bak<String> graph = new Graph_bak<>();
//        graph.addEdge("1", "2");
//        graph.addEdge("1", "3");
//        graph.addEdge("3", "4");
//        graph.addEdge("4", "5");
...
//        // 优化DAG
//        Optimizer<String> optimizer = new Optimizer<>(graph);
//        Graph_bak<String> optimizedGraph = optimizer.optimize();
//
//        optimizedGraph.printGraph();
//
//    }
