<template>
  <div class="container">
    ...
    <div class="flow-container-wrapper">
     ...
     <!-- 设置边信息弹窗 -->
    <el-dialog v-model="isEditDialogVisible" title="设置边信息" width="25%">
      <el-form :model="edgeForm" label-width="70px">
        <el-form-item label="条件值：">
          <el-input v-model="edgeForm.label" @keyup.enter="saveEdgeLabel" placeholder="请输入条件值"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
const onEdgeDoubleClick = (event: any) => {
  ...
  if (!edge || !edge.id) {
    throw new Error("无效的边对象");
  }
  // 打开弹窗
  edgeForm.value = {
    id: edge.id,
    label: edge.label ? edge.label.toString() : '', 
  };
...
const saveEdgeLabel = () => {
  const edgeIndex = elements.value.findIndex(e => e.id === edgeForm.value.id);
  if (edgeIndex === -1) {
   
    ElMessage.error("边未找到");
    return;
  } 
  // 创建一个新的边对象，确保引用发生变化
  const updatedEdge = {
  ...
  elements.value.splice(edgeIndex, 1, updatedEdge);
  // 强制重新渲染 VueFlow
  elements.value = [...elements.value];
  isEditDialogVisible.value = false;
  ElMessage.success("边标签已更新");
};
</script>
